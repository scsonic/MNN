...
ErrorCode BinaryBufExecution::SubgroupOnResize(const std::vector<Tensor*>& inputs,
    ...
    unit.kernel = runTime->buildKernel("binary_subgroup_buf", kernelName, buildOptions, openCLBackend->getPrecision(),
                                       inputs[0], output);
    ...
    if (input0_c_pack == 16 && input1_c_pack == 16) {
        mGlobalWorkSize = {(uint32_t)UP_DIV(outputShape[2], 4) * outputShape[1], (uint32_t)ROUND_UP(outputShape[3], 16),
        ...
        mLocalWorkSize =
            localWS3DDefault(mGlobalWorkSize, mMaxWorkGroupSize, openCLBackend->getOpenCLRuntime(), kernelName,
                             unit.kernel, openCLBackend->getCLTuneLevel(), "binary_subgroup_buf")
                .first;
    ...
    for (int i = 2; i < inputs.size(); ++i) {
        ...
        std::string kernelNameTmp = "binary_buf_c" + std::to_string(input0_c_pack_tmp) + "_c" +
                                    std::to_string(input1_c_pack_tmp) + "_c" + std::to_string(output_c_pack_tmp);
        unit.kernel = runTime->buildKernel("binary_subgroup_buf", kernelNameTmp, buildOptions,
                                           openCLBackend->getPrecision(), inputs[i], output);
        mMaxWorkGroupSize = static_cast<uint32_t>(runTime->getMaxWorkGroupSize(unit.kernel));
        ...
        if (input0_c_pack_tmp == 16 && input1_c_pack_tmp == 16) {
            ...
            mLocalWorkSize =
                localWS3DDefault(mGlobalWorkSize, mMaxWorkGroupSize, openCLBackend->getOpenCLRuntime(), kernelNameTmp,
                                 unit.kernel, openCLBackend->getCLTuneLevel(), "binary_subgroup_buf")
                    .first;
        ...
        if (op->type() == OpType_Eltwise) {
            switch (op->main_as_Eltwise()->type()) {
                case EltwiseType_SUM:
                    OPENCL_CREATOR_CHECK(new BinaryBufExecution(inputs, "in0+in1", op, backend));
                case EltwiseType_PROD:
                    OPENCL_CREATOR_CHECK(new BinaryBufExecution(inputs, "in0*in1", op, backend));
                case EltwiseType_SUB:
                    OPENCL_CREATOR_CHECK(new BinaryBufExecution(inputs, "in0-in1", op, backend));
                case EltwiseType_MAXIMUM:
                    OPENCL_CREATOR_CHECK(new BinaryBufExecution(inputs, "in0>in1?in0:in1", op, backend));
                default:
                    break;
            }
            return nullptr;
        ...
        if (op->type() == OpType_BinaryOp) {
            ...
            switch (op->main_as_BinaryOp()->opType()) {
                ...
                case BinaryOpOperation_MUL_SILU:
                    OPENCL_CREATOR_CHECK(new BinaryBufExecution(
                        inputs, "in0*(in1*native_recip((float4)1+native_exp(-in1)))", op, backend));
                ...
                case BinaryOpOperation_REALDIV:
                    OPENCL_CREATOR_CHECK(new BinaryBufExecution(
                        inputs,
                        "sign(in1)*in0/(fabs(in1)>(float4)((float)0.0000001)?fabs(in1):(float4)((float)0.0000001))", op,
                        backend));
                case BinaryOpOperation_MINIMUM:
                    OPENCL_CREATOR_CHECK(new BinaryBufExecution(inputs, "in0>in1?in1:in0", op, backend));
                case BinaryOpOperation_MAXIMUM:
                    OPENCL_CREATOR_CHECK(new BinaryBufExecution(inputs, "in0>in1?in0:in1", op, backend));
                case BinaryOpOperation_GREATER:
                    OPENCL_CREATOR_CHECK(
                        new BinaryBufExecution(inputs, "convert_float4(-isgreater(in0,in1))", op, backend));
                case BinaryOpOperation_LESS:
                    OPENCL_CREATOR_CHECK(
                        new BinaryBufExecution(inputs, "convert_float4(-isless(in0,in1))", op, backend));
                case BinaryOpOperation_LESS_EQUAL:
                    OPENCL_CREATOR_CHECK(
                        new BinaryBufExecution(inputs, "convert_float4(-islessequal(in0,in1))", op, backend));
                case BinaryOpOperation_GREATER_EQUAL:
                    OPENCL_CREATOR_CHECK(
                        new BinaryBufExecution(inputs, "convert_float4(-isgreaterequal(in0,in1))", op, backend));
                case BinaryOpOperation_EQUAL:
                    OPENCL_CREATOR_CHECK(
                        new BinaryBufExecution(inputs, "convert_float4(-isequal(in0,in1))", op, backend));
                case BinaryOpOperation_FLOORDIV:
                    OPENCL_CREATOR_CHECK(new BinaryBufExecution(
                        inputs,
                        "floor(sign(in1)*in0/"
                        "(fabs(in1)>(float4)((float)0.0000001)?fabs(in1):(float4)((float)0.0000001)))",
                        op, backend));
                case BinaryOpOperation_FLOORMOD:
                    OPENCL_CREATOR_CHECK(new BinaryBufExecution(
                        inputs,
                        "in0-floor(sign(in1)*in0/"
                        "(fabs(in1)>(float4)((float)0.0000001)?fabs(in1):(float4)((float)0.0000001)))*in1",
                        op, backend));
                case BinaryOpOperation_POW:
                    OPENCL_CREATOR_CHECK(new BinaryBufExecution(inputs, "pow(in0,in1)", op, backend));
                case BinaryOpOperation_SquaredDifference:
                    OPENCL_CREATOR_CHECK(new BinaryBufExecution(inputs, "(in0-in1)*(in0-in1)", op, backend));
                case BinaryOpOperation_ATAN2:
                    OPENCL_CREATOR_CHECK(new BinaryBufExecution(inputs,
                                                                "(in1==(float)0?(sign(in0)*(float4)(PI/2)):(atan(in0/"
                                                                "in1)+(in1>(float4)0?(float4)0:sign(in0)*(float)PI)))",
                                                                op, backend));
                case BinaryOpOperation_NOTEQUAL:
                    OPENCL_CREATOR_CHECK(
                        new BinaryBufExecution(inputs, "convert_float4(-isnotequal(in0,in1))", op, backend));
