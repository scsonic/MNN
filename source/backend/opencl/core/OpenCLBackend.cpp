...
bool CLRuntime::onMeasure(const std::vector<Tensor*>& inputs, const std::vector<Tensor*>& outputs, const MNN::Op* op,
                          Runtime::OpInfo& dstInfo) const {
...
Backend* CLRuntime::onCreate(const BackendConfig* config, Backend* origin) const {
    ...
    if (hint().weightMemoryPath.size() > 0 && mUseMmapPool) {
        ...
        if (mMmapPool.get() == nullptr) {
            ...
            mmap.reset(
                new OpenCLMmapAllocator(hint().weightMemoryPath.c_str(), prefix.c_str(), posfix.c_str(), autoRemove));
            mMmapPool.reset(new MmapPool(mmap, mOpenCLRuntime->context(), mOpenCLRuntime->commandQueue(),
                                         CL_MEM_READ_WRITE | CL_MEM_ALLOC_HOST_PTR, hint().useCachedMmap, mmapSize));
        }
    }
...
OpenCLBackend::OpenCLBackend(BackendConfig::PrecisionMode precision, BackendConfig::MemoryMode memory, int gpuMode,
                             const CLRuntime* runtime)
...
public:
    ...
    virtual ~CLReleaseExecutionBuffer() { mBufferPool->recycle(mNode); }
...
public:
    ...
    virtual ~CLMemReleaseBuffer() { mBufferPool->recycle(mBuffer); }
...
public:
    ...
    virtual ~CLMemReleaseImage() { mBufferPool->recycle(mBuffer); }
...
Backend::MemObj* OpenCLBackend::onAcquire(const Tensor* nativeTensor, StorageType storageType) {
    ...
    if (mMemType == BUFFER) {
        ...
        if (MNN_DATA_FORMAT_NC4HW4 == TensorUtils::getDescribe(nativeTensor)->dimensionFormat &&
            nativeTensor->dimensions() >= 2) {
        ...
        if (mOpenCLRuntime->isSupportedIntelSubgroup()) {
            ...
            size_t imageWidth = (size_t)ROUND_UP(UP_DIV(C, cPack), 2) *
                                ROUND_UP(pads.left + W + pads.right, 4); // C-round to 8,W-round to 4, for memory alloc
...
Execution* OpenCLBackend::onCreate(const std::vector<Tensor*>& inputs, const std::vector<Tensor*>& outputs,
    ...
    if (mMemType == IMAGE) {
        ...
        if (!valid) {
            ...
            for (auto t : inputs) {
                auto tensorShape = OpenCL::tensorShapeFormat(t);
                MNN_PRINT("input n:%d, h:%d, w:%d, c:%d\n", tensorShape[0], tensorShape[1], tensorShape[2],
                          tensorShape[3]);
            }
            for (auto t : outputs) {
                auto tensorShape = OpenCL::tensorShapeFormat(t);
...
void OpenCLBackend::onResizeBegin() {
    mOpenCLRuntime->setPrebuildResizeActive(true);
#ifndef ENABLE_OPENCL_TIME_PROFILER
    mOpenCLRuntime->setCommandQueueProfileEnable();
#endif
    // update mUseRecordableQueueSize if hint has changed
    const int commitNum = mCLRuntime->hint().encorderNumForCommit;
    uint32_t hintSize = commitNum >= 0 ? commitNum : 10;
...
ErrorCode OpenCLBackend::onResizeEnd() {
    ...
    mOpenCLRuntime->setPrebuildResizeActive(false);
    return NO_ERROR;
}
...
bool OpenCLBackend::_allocHostBuffer(int length, const Tensor* srcTensor) const {
    ...
    if (MNN_MEMORY_AHARDWAREBUFFER == memType) {
        if (mOpenCLRuntime->isSupportAHD()) {
            ...
            if (sharedMem == nullptr ||
                (sharedMem != nullptr && srcTensor->buffer().device != sharedMem->getSharedId())) {
                if (mOpenCLRuntime->getGpuType() == MALI) {
                    const cl_import_properties_arm properties[] = {CL_IMPORT_TYPE_ARM,
                                                                   CL_IMPORT_TYPE_ANDROID_HARDWARE_BUFFER_ARM, 0};
                    Backend::MemObj* SharedTmp = new CLSharedMemReleaseBuffer(
                        srcTensor->buffer().device,
                        new cl::Buffer(mOpenCLRuntime->context(), (cl_mem_flags)CL_MEM_READ_WRITE, properties,
                                       (void*)srcTensor->buffer().device, CL_IMPORT_MEMORY_WHOLE_ALLOCATION_ARM,
                                       &error));
                    ...
                    Backend::MemObj* SharedTmp = new CLSharedMemReleaseBuffer(
                        srcTensor->buffer().device,
                        new cl::Buffer(mOpenCLRuntime->context(),
                                       (cl_mem_flags)(CL_MEM_USE_HOST_PTR | CL_MEM_EXT_HOST_PTR_QCOM), 0, &myAHBmem,
                                       &error));
                    TensorUtils::setSharedMem(srcTensor, SharedTmp);
                } else {
...
#endif
    {
        ...
        mHostBuffer.second.reset(new cl::Buffer(mOpenCLRuntime->context(),
                                                (cl_mem_flags)(CL_MEM_READ_WRITE | CL_MEM_ALLOC_HOST_PTR),
                                                (size_t)length, NULL, &error));
        ...
        for (int c = 0; c < bufferShape[3]; c++) {
            for (int h = 0; h < bufferShape[1]; h++) {
                for (int w = 0; w < bufferShape[2]; w++) {
                    hostPtr[n * bufferShape[3] * bufferShape[1] * bufferShape[2] + c * bufferShape[1] * bufferShape[2] +
                            h * bufferShape[2] + w] =
                        tmpPtr[n * C_4 * bufferShape[1] * bufferShape[2] * 4 +
                               (c / 4) * bufferShape[1] * bufferShape[2] * 4 + h * bufferShape[2] * 4 + w * 4 + c % 4];
                }
            }
        }
    }
...
void CLRuntime::convertFromDevice(const Tensor* srcTensor, const Tensor* dstTensor, MNN_DATA_FORMAT data_format,
                                  int precision, int backend_memtype, bool svmFlag, int memtype) const {
#ifdef __ANDROID__
    if (MNN_MEMORY_AHARDWAREBUFFER == memtype) {
        convertBetweenAHDandCLmem(const_cast<Tensor*>(srcTensor), const_cast<Tensor*>(dstTensor), mOpenCLRuntime.get(),
                                  precision, backend_memtype, false, true);
        return;
    }
#endif
#ifndef MNN_OPENCL_BUFFER_CLOSED
            ...
            switch (data_format) {
                case MNN_DATA_FORMAT_NHWC:
                    OpenCL::convertNC4HW4OrNC16HW16BufferToNCHWOrNHWCBuffer(
                        srcTensor, const_cast<Tensor*>(dstTensor), "nc16hw16_buffer_to_nhwc_buffer",
                        mOpenCLRuntime.get(), precision, true, false, svmFlag);
                    break;
                ...
                case MNN_DATA_FORMAT_NC4HW4:
                    OpenCL::convertNC4HW4BufferBetweenNC16HW16Buffer(
                        srcTensor, const_cast<Tensor*>(dstTensor), "nc16hw16_buffer_to_nc4hw4_buffer",
                        mOpenCLRuntime.get(), precision, OutTrans, false, svmFlag, false, true);
                    break;
                default:
...
#endif
            OpenCL::convertBufferToBuffer(const_cast<Tensor*>(srcTensor), const_cast<Tensor*>(dstTensor),
                                          mOpenCLRuntime.get(), precision, precision, precision, false, true, true,
                                          svmFlag);
    } else
#endif /* MNN_OPENCL_BUFFER_CLOSED */
    {
        switch (data_format) {
            case MNN_DATA_FORMAT_NHWC:
                OpenCL::convertImageToNHWCBuffer(srcTensor, const_cast<Tensor*>(dstTensor), mOpenCLRuntime.get(),
                                                 precision, false, svmFlag);
                break;
            case MNN_DATA_FORMAT_NCHW:
                OpenCL::convertImageToNCHWBuffer(srcTensor, const_cast<Tensor*>(dstTensor), mOpenCLRuntime.get(),
                                                 precision, false, svmFlag);
                break;
            case MNN_DATA_FORMAT_NC4HW4:
                OpenCL::convertImageToNC4HW4Buffer(srcTensor, const_cast<Tensor*>(dstTensor), mOpenCLRuntime.get(),
                                                   precision, false, svmFlag);
                break;
            default:
                break;
...
void OpenCLBackend::copyFromDevice(const Tensor* srcTensor, const Tensor* dstTensor) const {
    ...
    bool directCopy =
        BUFFER == mMemType && (srcDimensionFormat == dstDimensionFormat || srcTensor->dimensions() <= 1) &&
        MNN::MNN_DATA_FORMAT_NC4HW4 != dstDimensionFormat && MNN_DATA_FORMAT_NC4HW4 != srcDimensionFormat &&
        (getDataType(srcTensor) == getDataType(dstTensor)) && memType != MNN_MEMORY_AHARDWAREBUFFER;
    ...
    // Convert format
    mCLRuntime->convertFromDevice(srcTensor, (const Tensor*)&interTensor, dstDimensionFormat, mPrecision, mMemType,
                                  false);
    mOpenCLRuntime->printEventTime();
...
void CLRuntime::convertToDevice(const Tensor* srcTensor, const Tensor* dstTensor, MNN_DATA_FORMAT data_format,
                                int precision, int backend_memtype, bool svmFlag, int memtype) const {
    // Format: Host -> OpenCL
#ifdef __ANDROID__
    if (MNN_MEMORY_AHARDWAREBUFFER == memtype) {
        convertBetweenAHDandCLmem(const_cast<Tensor*>(srcTensor), const_cast<Tensor*>(dstTensor), mOpenCLRuntime.get(),
                                  precision, backend_memtype, true, false);
        return;
    }
#endif
#ifndef MNN_OPENCL_BUFFER_CLOSED
    if (backend_memtype == BUFFER) {
        ...
        if (cPack == 16 && mOpenCLRuntime->isSupportedIntelSubgroup()) {
            if (MNN_DATA_FORMAT_NHWC == data_format) {
                OpenCL::converNCHWOrNHWCBufferToNC4HW4OrNC16HW16Buffer(
                    srcTensor, const_cast<Tensor*>(dstTensor), "nhwc_buffer_to_nc16hw16_buffer", mOpenCLRuntime.get(),
                    precision, true, false, svmFlag);
            } else if (MNN_DATA_FORMAT_NCHW == data_format) {
                OpenCL::converNCHWOrNHWCBufferToNC4HW4OrNC16HW16Buffer(
                    srcTensor, const_cast<Tensor*>(dstTensor), "nchw_buffer_to_nc16hw16_buffer", mOpenCLRuntime.get(),
                    precision, true, false, svmFlag);
            } else if (MNN_DATA_FORMAT_NC4HW4 == data_format) {
                OpenCL::convertNC4HW4BufferBetweenNC16HW16Buffer(
                    srcTensor, const_cast<Tensor*>(dstTensor), "nc4hw4_buffer_to_nc16hw16_buffer", mOpenCLRuntime.get(),
                    precision, InpTrans, false, svmFlag, true, false);
            } else {
...
#endif
            OpenCL::convertBufferToBuffer(const_cast<Tensor*>(srcTensor), const_cast<Tensor*>(dstTensor),
                                          mOpenCLRuntime.get(), precision, precision, precision, true, false, false,
                                          svmFlag);
    } else
#endif /* MNN_OPENCL_BUFFER_CLOSED */
    {
        if (MNN_DATA_FORMAT_NHWC == data_format) {
            OpenCL::convertNHWCBufferToImage(srcTensor, const_cast<Tensor*>(dstTensor), mOpenCLRuntime.get(), precision,
                                             false, svmFlag);
        } else if (MNN_DATA_FORMAT_NCHW == data_format) {
            OpenCL::convertNCHWBufferToImage(srcTensor, const_cast<Tensor*>(dstTensor), mOpenCLRuntime.get(), precision,
                                             false, svmFlag);
        } else if (MNN_DATA_FORMAT_NC4HW4 == data_format) {
            OpenCL::convertNC4HW4BufferToImage(srcTensor, const_cast<Tensor*>(dstTensor), mOpenCLRuntime.get(),
                                               precision, false, svmFlag);
        } else {
...
void OpenCLBackend::copyToDevice(const Tensor* srcTensor, const Tensor* dstTensor) const {
    ...
    if (BUFFER == mMemType && srcTensor->getType().code == halide_type_float &&
        mPrecision != BackendConfig::Precision_High && 1 == shape[0] * shape[1] * shape[2] * shape[3]) {
        needSize /= 2;
    ...
    // Covert format
    mCLRuntime->convertToDevice((const Tensor*)&interTensor, dstTensor, srcDimensionFormat, mPrecision, mMemType,
                                false);
}
...
void OpenCLBackend::copyBetweenDevice(const Tensor* srcTensor, const Tensor* dstTensor) const {
    ...
    if (MNN_FORWARD_CPU == srcMemtype && MNN_FORWARD_CPU == dstMemtype) {
        ...
        if (MNN_FORWARD_CPU != srcMemtype) {
            ...
            mCLRuntime->convertFromDevice(deviceTensor, hostTensor, data_format, mPrecision, mMemType, false,
                                          dstMemtype);
        }
    }
}

void CLRuntime::copyBetweenDevice(const Tensor* srcTensor, const Tensor* dstTensor, int precision,
                                  int backend_memtype) const {
    ...
    if (backend_memtype == BUFFER) {
        OpenCL::convertBufferToBuffer(const_cast<Tensor*>(srcTensor), const_cast<Tensor*>(dstTensor),
                                      mOpenCLRuntime.get(), input_precision, output_precision, precision, true, true);
    } else
#endif /* MNN_OPENCL_BUFFER_CLOSED */
        if (input_precision == output_precision) {
            ...
            mOpenCLRuntime.get()->commandQueue().enqueueCopyImage(
                openCLImage(srcTensor), openCLImage(dstTensor), {0, 0, 0}, {0, 0, 0},
                {(size_t)bufferShape[2] * UP_DIV(bufferShape[3], 4), (size_t)bufferShape[0] * bufferShape[1], 1});
        } else {
            OpenCL::convertImageToImage(const_cast<Tensor*>(srcTensor), const_cast<Tensor*>(dstTensor),
                                        mOpenCLRuntime.get(), input_precision, output_precision, precision);
        }
    return;
}
...
void* OpenCLBackend::allocMapTensorMemory(int length, bool svmFlag, cl_device_svm_capabilities svm_cap_) {
    ...
    if (svmFlag) {
        ...
        flags |= ((svm_cap_ & CL_DEVICE_SVM_FINE_GRAIN_BUFFER) && (svm_cap_ & CL_DEVICE_SVM_ATOMICS))
                     ? CL_MEM_SVM_ATOMICS
                     : 0;
...
void* OpenCLBackend::onMapTensor(Tensor::MapType mtype, Tensor::DimensionType dtype, const Tensor* srcTensor) {
    ...
    use_svm |= ((svm_cap_ & CL_DEVICE_SVM_COARSE_GRAIN_BUFFER) &&
                mOpenCLRuntime->getGpuType() == ADRENO); // support coarse grain svm and adreno gpu
    ...
    if (mUseSvm) { // CL version beyond 2.0 & support svm
        ...
        cl_int res = clEnqueueSVMMap(mOpenCLRuntime->commandQueue().get(), true, map_flag, svmPtr, needSize, 0, nullptr,
                                     nullptr);
...
bool OpenCLBackend::onUnmapTensor(Tensor::MapType mtype, Tensor::DimensionType dtype, const Tensor* dstTensor,
                                  void* mapPtr) {
#ifdef MNN_OPENCL_SVM_ENABLE
    auto svm_cap_ = mOpenCLRuntime->getSvmCapabilities();
    if (mUseSvm) { // CL version beyond 2.0 & support svm
...
class CLRuntimeCreator : public RuntimeCreator {
    ...
    virtual bool onValid(Backend::Info& info) const { return true; }
};
...
void OpenCLBackend::clearRecord() const {
    ...
    if (mUseRecordQueue && mDivideOpRecord) {
        for (int i = 0; i < mRecordings.size(); ++i) {
            ...
            cl_int res = mOpenCLRuntime->commandQueue().EnqueueRecordingQCOM(
                mRecordings[i].record, update_kernel_args.size(), update_kernel_args.data(), 0, nullptr,
                update_global_size.size(), update_global_size.data(), update_local_size.size(),
...
void OpenCLBackend::recordKernel2d(const std::shared_ptr<KernelWrap>& kernelW, const std::vector<uint32_t>& gws,
                                   const std::vector<uint32_t>& lws, RecordUpdateInfo* updateInfo) {
#if !defined(ENABLE_OPENCL_TIME_PROFILER) && defined(MNN_USE_LIB_WRAPPER)
    if (!mUseRecordQueue) {
        return;
    }
    ...
    if (lws[0] == 0 || lws[1] == 0) {
        res = mOpenCLRuntime->recordableQueue().enqueueNDRangeKernel(
            kernel, cl::NullRange, cl::NDRange(internalGlobalWS[0], internalGlobalWS[1]), cl::NullRange, nullptr,
            nullptr);
...
void OpenCLBackend::recordKernel3d(const std::shared_ptr<KernelWrap>& kernelW, const std::vector<uint32_t>& gws,
                                   const std::vector<uint32_t>& lws, RecordUpdateInfo* updateInfo) {
#if !defined(ENABLE_OPENCL_TIME_PROFILER) && defined(MNN_USE_LIB_WRAPPER)
    if (!mUseRecordQueue) {
        return;
...
void OpenCLBackend::setGpuMode(const int cl_mode_num) {
    ...
    mUseRecordQueue = ((cl_mode_num & MNN_GPU_RECORD_OP) || (cl_mode_num & MNN_GPU_RECORD_BATCH)) &&
                      mOpenCLRuntime->isSupportRecordQueue() && (mUseRecordableQueueSize > 0);
...
bool placeholder = []() {
    ...
    std::call_once(createOnce, []() { MNNInsertExtraRuntimeCreator(MNN_FORWARD_OPENCL, new CLRuntimeCreator, true); });
    return true;
}();
#else
